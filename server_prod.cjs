...
async function startServer() {
  ...
  FinancialJobs.start();
  const PORT = process.env.PORT ? Number(process.env.PORT) || process.env.PORT : 3e3;
  ...
  app.get("/api/support-info", async (req, res) => {
    try {
      const keys = ["SUPPORT_PHONE", "SUPPORT_PHONE_2", "SUPPORT_TELEGRAM", "SUPPORT_RUBIKA", "SUPPORT_BALE", "SUPPORT_EMAIL", "SUPPORT_CHANNELS_JSON"];
      ...
      res.status(500).json({ error: "\u062E\u0637\u0627 \u062F\u0631 \u062F\u0631\u06CC\u0627\u0641\u062A \u0627\u0637\u0644\u0627\u0639\u0627\u062A \u067E\u0634\u062A\u06CC\u0628\u0627\u0646\u06CC" });
    }
  });
  app.get("/api/terms", async (req, res) => {
    try {
      const keys = ["SUPPLIER_TERMS", "STORE_TERMS", "CUSTOMER_TERMS", "GENERAL_TERMS"];
      const settings = await prisma13.systemConfig.findMany({ where: { key: { in: keys } } });
      const map = {};
      settings.forEach((s) => {
        map[s.key] = s.value;
      ...
      res.status(500).json({ error: "\u062E\u0637\u0627 \u062F\u0631 \u062F\u0631\u06CC\u0627\u0641\u062A \u0642\u0648\u0627\u0646\u06CC\u0646" });
    }
  });
  app.get("/api/custom-code", async (req, res) => {
    try {
      const keys = ["CUSTOM_CODE_HEADER", "CUSTOM_CODE_FOOTER"];
      const settings = await prisma13.systemConfig.findMany({ where: { key: { in: keys } } });
      const map = {};
      settings.forEach((s) => {
        map[s.key] = s.value;
      ...
      res.status(500).json({ error: "\u062E\u0637\u0627 \u062F\u0631 \u062F\u0631\u06CC\u0627\u0641\u062A \u06A9\u062F\u0647\u0627\u06CC \u0633\u0641\u0627\u0631\u0634\u06CC" });
    }
  });
