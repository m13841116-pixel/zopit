...
export function SupplierAddProduct({
  ...
  const handleSubmit = async (submitStatus: 'DRAFT' | 'PENDING_APPROVAL' = 'PENDING_APPROVAL') => {
...
<div className="bg-background border-b border-subtle p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {!initialData?.id && (
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={onNavigateToConciergeImport || onNavigateToTickets}
              className="px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/20 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95"
            >
              <FileCheck className="w-3.5 h-3.5 text-amber-500" />
              <span>محصولاتم را شما وارد کنید (رایگان)</span>
            </button>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="max-w-6xl w-full my-auto max-h-[92vh] overflow-y-auto bg-card rounded-3xl p-4 md:p-6 border border-subtle shadow-2xl relative">
            <button
              ...
              className="absolute top-6 left-6 p-2 rounded-xl bg-surface hover:bg-subtle text-text-secondary border border-border-default transition-all cursor-pointer z-20"
              title="بستن"
            >
              <X className="w-5 h-5" />
            </button>
